  ...
  try {
    ...
    if (frontOnly) {
      const frontOnlySystem = [
        ...
        "- ALL output must be in English: nutrient names, ingredient names, entity names, signal text. Translate from any label language.",
    ...
    const extractSystem = [
      ...
      "• Map non-English names: IJzer→iron, Zink→zinc, Foliumzuur→folate, Vitamine B12→vitamin_b12, Jodium→iodine, Koper→copper, Chroom→chromium, Kalium→potassium, Mangaan→manganese, Selenium→selenium, Fosfor→phosphorus, Molybdeen→molybdenum, Biotine→biotin, Pantotheenzuur→vitamin_b5, Niacine→vitamin_b3, Riboflavine→vitamin_b2, Thiamine→vitamin_b1, Eisen→iron, Folsäure→folate, Jod→iodine, Fett→Fat, Kohlenhydrate→Carbohydrates, davon Zucker→Sugar, Eiweiß→Protein, Salz→Salt, davon gesättigte Fettsäuren→Saturated fat, Brennwert→Energy, Ballaststoffe→Fiber",
      "• IMPORTANT: The 'name' field must ALWAYS be in English, regardless of the label language. Translate: Fett→Fat, Eisen→Iron, Zink→Zinc, Kohlenhydrate→Carbohydrates, Eiweiß→Protein, Salz→Salt, davon Zucker→Sugar, etc.",
      ...
      "Only include entities explicitly mentioned in the transcription. ALL entity names and category entries must be in English.",
      "",
      "Ingredients list: split 'Ingredients:' or 'Other ingredients:' section by commas/semicolons. Up to 80 items. TRANSLATE all ingredient names to English (e.g. Erbsenprotein→Pea protein, Reisprotein→Rice protein, Kakaopulver→Cocoa powder, Sonnenblumenöl→Sunflower oil).",
      "",
