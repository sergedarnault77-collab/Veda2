    ...
    if (percentLabel !== null) {
      ...
      if (knownRef) {
        if (unit === "IU" && knownRef.iuEquiv) {
          ...
          if (converted !== null && converted > 0) dailyReference = converted;
        }
      }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(200).json(stub("OPENAI_API_KEY missing"));
    }
    ...
    if (!isDataImage(frontImageDataUrl)) {
      return res.status(200).json(stub("missing or invalid front image"));
    }
    ...
    if (approxBytesFromDataUrl(frontImageDataUrl) > maxBytesPerImage) {
      return res.status(200).json(stub("front image too large"));
    }
    for (const img of ingredientImages) {
      if (approxBytesFromDataUrl(img) > maxBytesPerImage) {
        return res.status(200).json(stub("ingredient image too large"));
      }
    }
    ...
    if (frontOnly) {
      const frontOnlySystem = [
        ...
        "- For nutrients: only include well-known, widely-documented amounts.",
        ...
        "- ALL output must be in English. Translate from any label language.",
      ...
      let frontR: globalThis.Response;
      try {
        frontR = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              { role: "user", content: [
                { type: "text", text: "Identify this product from the front photo. Return JSON with: productName, transcription (null), transcriptionConfidence (0.3), servingSizeG (number|null), nutritionPer ('serving' or 'unknown'), nutrients [{nutrientId, name, unit, amountToday, dailyReference, percentLabel}], ingredientsList [strings], categories {Sweeteners,Stimulants,Sugars,Calories,Vitamins,Minerals,Supplements,Other}, detectedEntities [strings], signals [{type,severity,confidence,headline,explanation,relatedEntities}]." },
        ...
        return res.status(200).json(stub("OpenAI request timed out (front-only). Try a clearer photo."));
      }
      clearTimeout(frontTimer);

      if (!frontR.ok) {
        const errText = await frontR.text().catch(() => "");
        return res.status(200).json(stub(`OpenAI error ${frontR.status}: ${errText.slice(0, 140)}`));
      }

      const frontResp = await frontR.json().catch(() => null);
      const frontOutText = frontResp?.choices?.[0]?.message?.content;
      if (!frontOutText) {
        return res.status(200).json(stub("OpenAI: no output (front-only)"));
      }

      let frontParsed: any = null;
      try { frontParsed = JSON.parse(frontOutText); } catch {
        return res.status(200).json(stub("OpenAI: invalid JSON (front-only)"));
      }
      ...
      return res.status(200).json(frontOnlyResp);
    }
    ...
    if (!anyTranscribed) {
      return res.status(200).json(stub("Couldn't read the label photos — try with more light or a steadier hand."));
    }
    ...
    const extractSystem = [
      ...
      "• Look for serving/portion info in the transcription.",
      "• Set 'servingSizeG' to the serving size in grams (number or null if unknown).",
      "• Set 'nutritionPer' to: '100g', 'serving', 'tablet', 'capsule', or 'unknown'.",
      "• CRITICAL: If the nutrition table header says 'per 100g' / 'pro 100g' / 'per 100 ml', set nutritionPer to '100g'.",
      "",
      "Nutrients guidance (populate 'nutrients' array):",
      "• CRITICAL: Extract EVERY nutrient row from the transcription. Include ALL of them.",
      ...
      "• Map non-English names to English.",
      "• IMPORTANT: The 'name' field must ALWAYS be in English.",
      ...
      "• percentLabel: if the transcription shows an explicit %, extract that number. Null if none.",
      ...
      "Only include entities explicitly mentioned in the transcription. ALL names must be in English.",
      "",
      "Ingredients list: split 'Ingredients:' section by commas/semicolons. Up to 80 items. TRANSLATE all to English.",
      "",
    ...
    try {
      ...
      clearTimeout(timer);
      return res.status(200).json(stub("Extraction timed out. The label was transcribed but processing took too long."));
    }
    clearTimeout(timer);

    if (!r.ok) {
      const errText = await r.text().catch(() => "");
      return res.status(200).json(stub(`OpenAI error ${r.status}: ${errText.slice(0, 140)}`));
    }

    const resp = await r.json().catch(() => null);
    const outText = resp?.choices?.[0]?.message?.content || extractOutputText(resp);
    if (!outText) {
      return res.status(200).json(stub("OpenAI: no output"));
    }

    let parsed: any = null;
    try { parsed = JSON.parse(outText); } catch {
      return res.status(200).json(stub("OpenAI: invalid JSON output"));
    }
    ...
    return res.status(200).json(stub(`handler error: ${String(e?.message || e).slice(0, 120)}`));
  }
}
